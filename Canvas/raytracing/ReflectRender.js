var ReflectRender = function(camera, screen, scene, lights, canvas){
	this.camera = camera;
	this.screen = screen;
	this.scene = scene;
	this.lights = lights;
...
function reflect(ray, scene, intersect){
	if(ray.weight < 0.2){

	}else{
		var intersectResult = this.scene.intersect(ray);
		var w = ray.weight * 0.8;
		ray = getReflectedRay(ray, intersectResult);
		ray.weight = w;
		reflect(ray, scene, intersectResult);
	}
}

function getReflectedRay(ray, result){
	var v = ray.direction.subtract(2*result.normal.multiply(ray.direction.dot(result.normal)));
	return new Ray(result.position, v.normalize());
}