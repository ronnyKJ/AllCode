...
方便地完成对单表的CURD操作<P>
 ...
 <table cellpadding=2 cellspacing=2>
  ...
  <td></td>
	<td> <div id="list" >
	<?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): ++$i;$mod = ($i % 2 )?><div id="div_<?php echo ($vo["id"]); ?>" class="result" style='font-weight:normal;<?php if(($key%2)  ==  "1"): ?>background:#ECECFF<?php endif; ?>'><div style="border-bottom:1px dotted silver"><?php echo ($vo["title"]); ?>  [<?php echo ($vo["email"]); ?> <?php echo (date('Y-m-d H:i:s',$vo["create_time"])); ?>] <br/><input type="button" value="编辑" class="small button" onClick="edit(<?php echo ($vo["id"]); ?>)"> <input type="button" value="删除" class="small button" onClick="del(<?php echo ($vo["id"]); ?>)"> </div>
	<div class="content"><?php echo (nl2br($vo["content"])); ?></div></div><?php endforeach; endif; else: echo "" ;endif; ?></div>
