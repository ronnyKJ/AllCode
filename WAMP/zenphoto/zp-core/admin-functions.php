...
function printLoginForm($redirect=null, $logo=true) {
	...
	$mails = array();
	if (!empty($requestor)) {
		$admin = $_zp_authority->getAnAdmin('`user`=', $requestor);
...
function print_language_string_list($dbstring, $name, $textbox=false, $locale=NULL, $edit='', $wide=TEXT_INPUT_SIZE, $ulclass='language_string_list', $rows=6) {
	...
	if (getOption('multi_lingual')) {
			...
			if (isset($strings[$key])) {
				...
				if (!empty($string)) {
					...
					<li>
						...
						<?php
						if ($textbox) {
							echo "\n".'<textarea name="'.$name.'_'.$key.'"'.$edit.' cols="'.$wide.'"	rows="'.$rows.'">'.htmlspecialchars($string,ENT_QUOTES,$charset).'</textarea>';
						} else {
		...
		if ($textbox) {
			echo '<textarea'.$groupid.' name="'.$name.'_'.$locale.'"'.$edit.' cols="'.$wide.'"	rows="'.$rows.'">'.htmlspecialchars($dbstring,ENT_QUOTES,$charset).'</textarea>';
		} else {
