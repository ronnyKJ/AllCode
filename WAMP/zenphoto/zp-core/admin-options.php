			...
			<table class="bordered">
				...
				<tr>
					...
					<td width="350">
						...
						<br clear="all" />
						<p class="notebox"><?php printf(gettext('Highlighted languages are not current with Zenphoto Version %s. (The version Zenphoto of the out-of-date language is shown in braces.) Please check the <a href="http://www.zenphoto.org/trac/report/9?sort=summary">translation tickets</a> for new and updated language translations.'),$zpversion);?></p>
					...
					<td>
						<p><?php echo gettext("You can disable languages by unchecking their checkboxs. Only checked languages will be available to the installation."); ?></p>
					...
					<td>
						...
						if (getOption('gallery_security')=='public') {
							...
							<p class="public_gallery"<?php if (getOption('gallery_security') == 'private') echo ' style="display:none"'; ?>>
								<label>
									<?php
									if (getOption('gallery_user') || getOption('search_user') || getOption('protected_image_user')) {
									...
									<input type="checkbox" name="login_user_field" id="login_user_field"
															value="1" <?php echo checked('1', getOption('login_user_field')); ?><?php echo $disable; ?> />
	...
	<form action="?action=saveoptions" method="post" autocomplete="off">
		...
		if (count($themelist) == 0) {
			...
			<tr>
				...
				<td>
					<input type="text" size="3" name="thumb_size" value="<?php echo $ts; ?>" />
				</td>
				...
				<td>
					...
					<span class="nowrap">
						<?php printf(gettext('%s%% left &amp; right'),
							'<input type="text" size="3" name="thumb_crop_width" id="thumb_crop_width" value="'.$cl.'" />') ?>
					</span>&nbsp;
					<span class="nowrap">
						<?php printf(gettext('%s%% top &amp; bottom'),
							'<input type="text" size="3" name="thumb_crop_height" id="thumb_crop_height"	value="'.$ct.'" />'); ?>
					</span>
				</td>
				<td>
