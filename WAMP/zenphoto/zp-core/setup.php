...
if (isset($_POST['db'])) { //try to update the zp-config file
	...
	if (isset($_POST['db_software'])) {
		updateConfigItem('db_software', $_POST['db_software']);
	}
	if (isset($_POST['db_user'])) {
		updateConfigItem('mysql_user', $_POST['db_user']);
	}
	if (isset($_POST['db_pass'])) {
		updateConfigItem('mysql_pass', $_POST['db_pass']);
	}
	if (isset($_POST['db_host'])) {
		updateConfigItem('mysql_host', $_POST['db_host']);
	}
	if (isset($_POST['db_database'])) {
		updateConfigItem('mysql_database', $_POST['db_database']);
	}
	if (isset($_POST['db_prefix'])) {
		updateConfigItem('mysql_prefix', $_POST['db_prefix']);
	}
}
...
if ($connection) {
	...
	if ($cfg) {
		...
		if (zp_loggedin(ADMIN_RIGHTS)) {
			...
			<p>'.sprintf(gettext('Set File/Folder permissions to %s.'),permissionsSelector($permission_names, $chmod)).'</p></form>';
		} else {
						...
						$(document).ready(function() {
							...
							image.onload = function() {
									...
									if (!getOption('UTF8_image_URI')) {
										?>
										$('#UTF8_uri_warn').html('<?php echo gettext('You should enable the URL option <em>UTF8 image URIs</em>'); ?>');
										$('#UTF8_uri_warn').show();
										<?php
									}
									?>
							...
							image.onerror = function() {
								...
								if (getOption('UTF8_image_URI')) {
									...
									$('#UTF8_uri_warn').html('<?php echo gettext('You should disable the URL option <em>UTF8 image URIs</em>'); ?>');
									$('#UTF8_uri_warn').show();
									<?php
								}?>
							};
