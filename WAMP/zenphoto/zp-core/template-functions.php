...
function printSearchForm($prevtext=NULL, $id='search', $buttonSource=NULL, $buttontext='', $iconsource=NULL, $query_fields=NULL, $object_list=NULL,$reseticonsource=NULL) {
	...
	<div id="search"><!-- search form -->
		...
		<form method="post" action="<?php echo WEBPATH.$searchurl; ?>" id="search_form">
			...
			<?php } ?>
			<span style="display:<?php if ($searchwords) echo 'inline'; else echo 'none';?>" id="reset_search"><span class="nowrap"><a href="javascript:reset();" title="<?php echo gettext('Clear search'); ?>"><img src="<?php echo $reseticonsource; ?>" alt="<?php echo gettext('Reset search'); ?>" /></a></span></span>
	...
	if (empty($titlelink)) {
		switch($_zp_gallery_page) {
			...
			case 'news.php':
				...
				break;
			case 'pages.php':
				if($_zp_current_zenpage_page->checkAccess($hint, $show)) {
