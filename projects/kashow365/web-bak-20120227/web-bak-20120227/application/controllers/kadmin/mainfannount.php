<?php
class MainFannount extends CI_Controller {
 ...
 function index()
 {
	...
	/////////////////////////////////////////////////////
 	// get DB entity Count
	$totalRowsCount = $this->MainFannountModel->GetEntityCount($where);
		
	/////////////////////////////////////////////////////
	// select db
	$perpage = '40';
 	$rows = $this->MainFannountModel->GetEntityByPage($where, $orderby, $perpage, $pageIndex);
	...
	$this->load->view('kadmin/kainfo/mainfannoutindex.php', $data);
 }
 ...
 function edit($id='')
 {
 	...
 	// DB操作
	$entity = $this->MainFannountModel->GetEntityById($id);
 ...
 function doadd()
 {
	...
	$this->form_validation->set_rules('content', '公告内容', 'trim|required|callback_max_length500');
	...
	else// 无错可操作
	{
		// DB操作
		if($formdata['id'] == '')
			$result = $this->MainFannountModel->Insert($formdata);
		else
		{
			$nowTime=date("Y-m-d H:i:s");
			$entity = $this->MainFannountModel->GetEntityById($formdata['id']);
			$entity['content'] = $formdata['content'];
			$entity['state'] = $formdata['state'];
			$entity['updateDateTime'] =  $nowTime;
			$result = $this->MainFannountModel->Update($entity);
		}
		...
		else 
		{
			...
			$data = array(
				...
				,'nexturl' => site_url("kadmin/mainfannount/add")
			);
			FormValidation::GotoOperMsgPage($data, $this);
		}
	}
...
function doUpState($id,$state)
 ...
 function d($id='')
 {
 	...
 	// DB操作
	$entity = $this->MainFannountModel->Delete($id);
 ...
 function ds($ids='')
 {
 	...
 	// DB操作
	$result = $this->MainFannountModel->Deletes($idsArray);
		
 	if($result=="1") // 操作成功
	{
		// 进入操作信息提示页  - 协带参数
		$this->GotoOperMsgPage('多个公告删除成功！');
	}
	else 
	{
		// 进入操作信息提示页 
 ...
 function RestoreFormDate($formdata)
 {
	...
	$this->load->view('kadmin/kainfo/mainfannoutadd.php', $data);
 }
 /* End public */
