<?php
class MainFCardForActivity extends CI_Controller {
 function __construct()
 {
  ...
  $this->load->model('kadmin/kcard/MainfCardForActivity_model','MainfCardForActivityModel');
 }
 ...
 function index()
 {
 	...
 	else
	{
 	...
 	/////////////////////////////////////////////////////
 	// get DB entity Count
 	$totalRowsCount = $this->MainfCardForActivityModel->GetEntityCount($where);
	
 	/////////////////////////////////////////////////////
	// select db
	$perpage = '20';
 	$rows = $this->MainfCardForActivityModel->GetEntityByPage($where, $orderby, $perpage, $pageIndex);
 	#var_dump($rows);	
	...
	#var_dump($data);
	$this->load->view('kadmin/kcard/mainfcardforactivity.php', $data);
...
function doUpState($id,$state)
 {
	...
	if($id != '' && $state != '')
	{
		#$entity = $this->MainfCardForActivityModel->GetEntityById($id);
		$entity['id'] = $id;
		$entity['state'] = $state == '0' ? '3' : '0'; 
		$result = $this->MainfCardForActivityModel->Update($entity);
	}
	...
	if($result=="1") // 操作成功
	{
		...
		$data = array(
			'title' => '操作成功'
			,'message' => '修改“卡”发布状态成功！'
			,'nexturl' => site_url("kadmin/mainfcardforactivity")
		);
	...
	else 
	{
		...
		$data = array(
			...
			,'nexturl' => site_url("kadmin/mainfcardforactivity/add")
		);
		FormValidation::GotoOperMsgPage($data, $this);
	}
 }
 ...
 function d($id='')
 {
 	...
 	// DB操作
	$entity = $this->MainfCardForActivityModel->Delete($id);
 ...
 function ds($ids='')
 {
 	...
 	// DB操作
	$result = $this->MainfCardForActivityModel->Deletes($idsArray);
