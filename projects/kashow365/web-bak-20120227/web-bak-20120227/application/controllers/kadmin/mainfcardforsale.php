<?php
class MainFCardForSale extends CI_Controller {
 function __construct()
 {
  ...
  $this->load->model('kadmin/kcard/MainfCardForSale_model','MainfCardForSaleModel');
 }
 ...
 function index()
 {
 	...
 	else
	{
 	...
 	/////////////////////////////////////////////////////
 	// get DB entity Count
 	$totalRowsCount = $this->MainfCardForSaleModel->GetEntityCount($where);
	
 	/////////////////////////////////////////////////////
	// select db
	$perpage = '20';
 	$rows = $this->MainfCardForSaleModel->GetEntityByPage($where, $orderby, $perpage, $pageIndex);
 	#var_dump($rows);	
	...
	#var_dump($data);
	$this->load->view('kadmin/kcard/mainfcardforsale.php', $data);
...
function doUpState($id,$state)
 {
	...
	if($id != '' && $state != '')
	{
		#$entity = $this->MainfCardForSaleModel->GetEntityById($id);
		$entity['id'] = $id;
		$entity['state'] = $state == '1' ? '3' : '1'; 
		$result = $this->MainfCardForSaleModel->Update($entity);
	}
	...
	if($result=="1") // 操作成功
	{
		...
		$data = array(
			'title' => '操作成功'
			,'message' => '修改“卡”发布状态成功！'
			,'nexturl' => site_url("kadmin/mainfcardforsale")
		);
	...
	else 
	{
		...
		$data = array(
			...
			,'nexturl' => site_url("kadmin/mainfcardforsale/add")
		);
		FormValidation::GotoOperMsgPage($data, $this);
	}
 }
 ...
 function d($id='')
 {
 	...
 	// DB操作
	$entity = $this->MainfCardForSaleModel->Delete($id);
 ...
 function ds($ids='')
 {
 	...
 	// DB操作
	$result = $this->MainfCardForSaleModel->Deletes($idsArray);
