<?php
class MainFshopnews extends CI_Controller {
 ...
 function index()
 {
	...
	#var_dump($data);
	$this->load->view('kadmin/news/mainfshopnews.php', $data);
	...
	else// 无错可操作
	{
		...
		if($result=="1") // 操作成功
		{
			...
			$data = array(
				'title' => '操作成功'
				,'message' => '“最新活动”发布成功！'
				,'nexturl' => site_url("kadmin/mainfshopnews")
			);
			FormValidation::GotoOperMsgPage($data, $this);
		}
		else 
		{
			...
			$data = array(
				'title' => '操作失败'
				,'message' => '“最新活动”发布出错：' + $result
				,'nexturl' => site_url("kadmin/mainfshopnews/add")
			);
			FormValidation::GotoOperMsgPage($data, $this);
		}
	}
...
function doUpState($id,$state)
 ...
 function RestoreFormDate($formdata)
 {
	...
	#var_dump($data);
	$this->load->view('kadmin/news/mainfshopnewsadd.php', $data);
 }
 /* End public */
