<?php
class MainFshopnews2 extends CI_Controller {
 ...
 function index()
 {
	...
	#var_dump($data);
	$this->load->view('kadmin/news/mainfshopnews2.php', $data);
 }
 ...
 function doadd()
 {
	...
	else// 无错可操作
	{
		...
		else 
		{
			...
			$data = array(
				'title' => '操作失败'
				,'message' => '“打折快报”发布出错：' + $result
				,'nexturl' => site_url("kadmin/mainfshopnews2/add")
			);
			FormValidation::GotoOperMsgPage($data, $this);
		}
	}
...
function doUpState($id,$state)
 ...
 function ds($ids='')
 {
	...
	{
		// 进入操作信息提示页  - 协带参数
		$this->GotoOperMsgPage('多个“打折快报”删除成功！');
	}
	else 
	{
		// 进入操作信息提示页 
 ...
 function RestoreFormDate($formdata)
 {
	...
	#var_dump($data);
	$this->load->view('kadmin/news/mainfshopnewsadd2.php', $data);
 }
 /* End public */
