<?php
class MainFBaseInfoIF_model extends CI_Model {
    ...
    function Delete($id)
    {
    	...
    	$DBCommOper->Delete(self::tableName, $id);
       	...
       	// DEL原图片
       	KXFile::DeleteFile($this->config->item('UpPathFlashIndex'), $entity['value1']);
    ...
    function Deletes($ids)
    {
    	...
    	if($result == '1')// DB deleted OK then delete files
    	{
			...
			{
				KXFile::DeleteFile($this->config->item('UpPathFlashIndex'), $row->value1);
			}
    	}
     ...
     * 返回$result: 集合
     */
    function GetEntityByPageForView($where='', $order='orderNum DESC ' , $pageSize=1, $pageIndex=1, $viewName)
    {
    	$DBCommOper = new DBCommOper();    	
    	return $DBCommOper->GetEntityByPage($viewName, $where, $order , $pageSize, $pageIndex);
    }
