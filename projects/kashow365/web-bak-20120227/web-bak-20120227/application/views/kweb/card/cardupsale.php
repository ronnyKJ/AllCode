...
function Finish(success,imagename){
	  ...
	  if (success == 1){
		$("#idProcess").hide();
		$("#userfile").show();
		$("#tempimg").attr("src","<?php echo site_url('card/carduppic?ui=');?>"+ imagename+"&rand="+timenow);
		$("#tempimg").show();
		$("#isupi").val("1");
...
function doadd(){
	// 清空默认值
	if($("#txtremarks").val() =="例如：卡号，使用规则等"){$("#txtremarks").val("");}
...
<div class="bigcont">
  ...
  <div class="leftcont4">
        ...
        <div class="midbut">
			<form id="uploadForm" action="<?php echo site_url("card/doupphoto");?>" 
  ...
  <div class="rightcont4">
	<iframe id="tempimg" src="<?php echo site_url('card/carduppic?ui=5ab7585f9750dcb2ce6539145365434f.jpg');?>" width="700px" height="370px"
		frameborder="0" scrolling="no" marginheight="0" marginwidth="0" style="display:none; padding-bottom:20px;"></iframe>
  <form id="formcard" name="formcard" action="<?php echo site_url("card/doaddsale");?>" method="post" target="_blank">
...
<table width="100%" border="0" cellspacing="0" cellpadding="0">
  ...
  <tr>
    <td class="pdtop30" align="center"><label>
	  <?php if($id==''){?>
      <input class="seabut" type="button" value="继续添加" onclick="location.href='<?php echo site_url("card/upsale");?>'" name="btnN" id="btnN" />　
	  <?php } ?>
