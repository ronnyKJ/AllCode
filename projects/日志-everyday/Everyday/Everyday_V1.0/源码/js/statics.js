...
schema:function(data){
	...
	var c = $("#"+containerID).append('<canvas>浏览器不支持Canvas...</canvas>').find("canvas").get(0);
	/**************************************************************************数据数组******************************/
	event = data.event||[];
	items = data.items||[];
	histogram = {
		...
		histoNoteMarginBottom : data.histoNoteMarginBottom||8,//色块标记顶边距
		/*****************************************************************************文字说明***************************/
	...
	function draw(containerID, canvasDOM)
	{
		...
		with(ctx){
			...
			stroke();
			//等高线左侧数字
			var w = $("#"+containerID).width($(canvas).width()).width();
			...
			for(var k=0; k<=histogram.graduationLevel; k++)
			{
				...
				$("#"+containerID).prepend('<span>' + tmp_l.substring(0,histogram.leftCharNum) + '</span>');
				var l = $("#"+containerID+" span").eq(0).height()/2;
				$("#"+containerID+" span").eq(0).css({
			...
			for(var g=0; g<histogram.groupSum; g++)
			{
				...
				//下侧文字描述
				$("#"+containerID).prepend('<span>' + event[g][0] + '</span>');
				var e = $("#"+containerID+" span:first-child");
				e.css({
			...
			for(var it=0; it<histogram.histoSum; it++)
			{
				...
				$("#"+containerID).prepend('<span>' + items[it] + '</span>');
				var n = $("#"+containerID+" span:first-child");
				n.css({
			...
			$("#"+containerID).prepend('<span style="right:' + histogram.leftWordMarginLeft + 'px; top:' + histogram.leftWordMarginTop + 'px;">' + histogram.leftWord + '</span>');
			$("#"+containerID).prepend('<span style="right:' + histogram.bottomWordMarginRight + 'px; top:' + histogram.bottomWordMarginTop + 'px;">' + histogram.bottomWord + '</span>');
			}
	}
	histogram.otherAttr();
